...
sycl::event MatrixMatrixOperations::matrixMatrixStep_optimizedCPU_subgroup(sycl::queue& queue, conf::fp_type* A, const int blockID, const int blockRow, const int blockStart, const int blockCount, const int blockCountXY) {
    const int wgSize_xy = conf::workGroupSizeGEMM_xy;
    if (conf::matrixBlockSize % wgSize_xy != 0) {
        throw std::runtime_error("xy work-group dimension for matrix multiplication must divide matrix block size");
    }
    ...
    sycl::event event = queue.submit([&](sycl::handler& h) {
        h.parallel_for(kernelRange, [=](auto& nd_item) {
            ...
            auto simd_cu = nd_item.get_sub_group();
            int simd_id = simd_cu.get_local_id()[0];
            int simd_size = simd_cu.get_local_range()[0];

            conf::fp_type value_simd_cu = 0.0;

            printf("%i \n", simd_size);

            for (int k = simd_id; k < static_cast<int>(matrixBlockSize); k+=simd_size) {
                // A = A - B * C^T
                value_simd_cu += A[blockStartIndex_B + i * matrixBlockSize + k] * A[blockStartIndex_C + j * matrixBlockSize + k];
            }

            conf::fp_type value = sycl::reduce_over_group(simd_cu, value_simd_cu, plus<conf::fp_type>());
