...
public:
     ...
     * It can either perform the update on all blocks that have to be updated or only either on the upper or lower part.
     *
     * The kernel launched by this function is optimized for execution on GPUs by using local memory.
     *
     * The method launches a kernel on the device asynchronously and returns after that.
     * One has to wait to ensure correctness.
     *
     ...
     *
     * The kernel launched by this function is optimized for execution on GPUs.
     * In addition to the optimizations performed by matrixMatrixStep_optimizedGPU this kernel broadcasts some values for the index calculation.
     *
     *
     * The method launches a kernel on the device asynchronously and returns after that.
     * One has to wait to ensure correctness.
     ...
     *
     * The kernel launched by this function is optimized for execution on GPUs.
     * In addition to the optimizations performed by matrixMatrixStep_optimizedGPU2 this kernel leverages 4x4 tiling per work-item.
     *
     *
     * The method launches a kernel on the device asynchronously and returns after that.
     * One has to wait to ensure correctness.
     ...
     *
     * The kernel launched by this function is optimized for execution on GPUs.
     * In contrast to matrixMatrixStep_optimizedCPU the parallelization is only performed over the rows of the matrix blocks
     * which improves performance for large matrices.
     *
     * The method launches a kernel on the device asynchronously and returns after that.
     * One has to wait to ensure correctness.
     *
     ...
     * @return a sycl event of the kernel execution
     */
    static sycl::event matrixMatrixStep_optimizedCPU2(sycl::queue& queue, conf::fp_type* A, int blockID, int blockRow,
                                                      int blockStart, int blockCount, int blockCountXY);
};
